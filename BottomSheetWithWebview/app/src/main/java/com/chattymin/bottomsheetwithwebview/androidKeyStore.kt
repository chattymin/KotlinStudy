...
// API 23 이하인 디바이스를 위해 Deprecated code 사용
    ...
    fun encrypt(plainText: String): String {
        Log.e("error", "encrypt: 1", )
        if (!_isSupported) {
            return plainText
        }
        Log.e("error", "encrypt: 2", )
        ...
        val base64EncryptedBytes = Base64.encodeToString(encryptedBytes, Base64.DEFAULT)
        return base64EncryptedBytes
    }


    fun decrypt(base64EncryptedCipherText: String): String {
        Log.e("error", "encrypt: 3", )
        if (!_isSupported) {
            return base64EncryptedCipherText
        }
        Log.e("error", "encrypt: 4", )
        val cipher = Cipher.getInstance(CIPHER_ALGORITHM).apply {
